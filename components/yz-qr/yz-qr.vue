<template>
	<view class="qrcode">
		<canvas :style="{width:size+ 'px', height:size+ 'px', background:bgc}" :canvas-id="canvasId"></canvas>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			couponQrCode() {
				...
				this.$emit('update:qrStart')
				new qrCode(this.canvasId, {
					text: this.text,
					width: this.size,
					height: this.size,
